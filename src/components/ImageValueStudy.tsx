...
export function ImageValueStudy({ imageUrl, headingLevel = 3 }: { imageUrl: string | null; headingLevel?: 3 | 4 }) {
  ...
  // Nothing is measured until an image is uploaded.
  const sourceKey = imageUrl ?? ""
    ...
    <section className="image-value-study" aria-labelledby={`${id}-heading`}>
      ...
      <div className="ivs-layout">
        <div className="ivs-image-side">
          ...
          <div className="ivs-preview" aria-busy={Boolean(imageUrl) && (!currentLoad || currentLoad.status === "loading")}>
            {!imageUrl ? <div className="ivs-message ivs-empty" role="status">{label("이미지를 올리면 명암별 면적을 볼 수 있어요.", "Upload an image to see its light and dark areas.")}</div>
              : frame ? <canvas ref={canvasRef} role="img" aria-label={`${label("전체 이미지", "Whole image")} · ${selectedBand === null ? modes.find(item => item.value === mode)?.text : `${bands[selectedBand]} ${label("영역 강조", "area highlighted")}`}`} />
              : currentLoad?.status === "error" ? <div className="ivs-message" role="status"><p>{label("이미지를 읽지 못했어요. 다시 시도해주세요.", "Could not read the image. Please retry.")}</p><button type="button" onClick={() => setAttempt(value => value + 1)}>{label("다시 시도", "Retry")}</button></div>
        ...
        <div className="ivs-controls">
          ...
          <p className="ivs-selection-note" role="status">{frame && !canMeasure
            ? label("불투명도 50% 이상인 픽셀이 없어요. 다른 이미지를 사용해보세요.", "No pixels at 50% opacity or above. Try another image.")
