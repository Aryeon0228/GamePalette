...
export function ColorLabTools({ hex, mode, onSelectColor, onAddColors, overview = false, extraCard, analysisSection = "all" }: ColorLabToolsProps) {
  ...
  const composeOptions: Array<{ value: ComposeTool; label: string }> = [
    { value: "shading", label: label("회화 명암", "Painterly ramp") },
  ...
  const analyzeOptions: Array<{ value: AnalyzeTool; label: string }> = [
    ...
    { value: "meaning", label: label("이름 · 인상", "Name & impression") },
  ]
  ...
  const renderOverviewFormats = (formats: ColorFormat[]) => <div className="overview-formats">{formats.map(fmt => {
    const value = formatColor(color, fmt)
    const display = fmt === "HEX" ? value : value.replace(/^[a-z]+\(/i, "").replace(/\)$/, "").replace(/,\s*/g, " ")
    return <div key={fmt} title={`${t(`fmtDesc.${fmt}`)} · ${value}`}><span>{fmt}</span><code>{display}</code>{renderCompactCopy(value, `overview-format-${fmt}`)}</div>
  })}</div>
        ...
        <section className="overview-compose-group" aria-label={label("배색", "Color combinations")}>
          ...
          <div className="overview-compose-grid">
            ...
            {renderCompactCard(t("gradientTitle"), <>
              <p className="overview-purpose">{t("gradientSub")}</p>
            ...
            {renderCompactCard(t("coldwarmTitle"), <>
              ...
              <div className="overview-coldwarm">
  ...
  return (
    <div className="color-lab-tools space-y-6">
      ...
      {mode === "compose" && composeTool === "shading" && (
        <ToolSection title={t("shadingTitle")} subtitle={t("shadingSub")}
        ...
        >
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">{label("색상칩으로 다음 색을 탐색하세요. 조명에 따른 표면색 변화는 색 · 빛 실험에서 비교할 수 있어요.", "Select a swatch to explore it. Compare how lighting changes surface colors in Color & light experiments.")}</p>
        </ToolSection>
      )}
